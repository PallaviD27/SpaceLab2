...
'''This function implements Modulation on the encoded bit sequence to either 16-QAM or 16-PSK.
Takes the following inputs - 
CodeWord - The encoded sequence after (7,4) Hamming encoding application,
Switch_mod - For selection of the modulating scheme - '0' for 16 QAM and '1' for 16 PSK, and
Switch_graph - To display the graphical representation of output; takes ON or OFF as values
'''
...
# Plot the constellation diagram
        ...
        plt.title("16-QAM Constellation Diagram with Gray Mapping")
        plt.xlabel("In-phase (I)*(1/sqrt(10))")
        plt.ylabel("Quadrature-phase (Q)*(1/sqrt(10))")
...
# Displaying the in-phase and quadrature phase values corresponding to a set of 4 bits.
        for p in range(Code_length // 4):
            ModSetp = CodeWord[p * 4:(p + 1) * 4] # Split the encoded bit stream into sets of 4
            print(f'Modulating Set {p}: {ModSetp}') # Print for verification
            ...
            if ModSetp_binary in QAM_Mapping:
                ...
                print(f"Match found for Modulating Set {ModSetp_binary}: I = {I}, Q = {Q}")
...
# Print Constellation diagram for 16-PSK
        ...
        np.set_printoptions(suppress=True, precision=6)  # Print option to suppress the scientific notation for sin and cosine
